...
```
DATABASE_URL="postgres://powerbase_auth_admin:root@localhost:5432/postgres"
```
...
Also, you must generate a JWT with the signature which has the `powerbase_admin` role (or one that is specified in `GOTRUE_JWT_ADMIN_ROLES`).
...
```
// file: test.env
DATABASE_URL="postgres://powerbase_auth_admin:root@localhost:7432/postgres" 👈 set to your port
```

```
// file: migrate.sh
export GOTRUE_DB_DATABASE_URL="postgres://powerbase_auth_admin:root@localhost:7432/$DB_ENV"
```
